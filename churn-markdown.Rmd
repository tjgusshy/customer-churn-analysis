...
library(kableExtra)
library(pacman)
p_load(tidyverse,lubridate, janitor,knitr, kableExtra,patchwork)

customer_churn <- read_csv("~/customer churn/telecom_customer_churn.csv") %>% 
  clean_names()
```

 `r text_spec("Introduction",bold=T,font_size = 20,color="Blue")`

 
This is a Telecommunication company that provides phone and internet services to `r text_spec(nrow(customer_churn),bold= T,color = "green",font_size = 10 )` customers in California, this project will focus on knowing why customers are leaving the company so that the company will be able to use insights from this analysis to prevent future churn.
